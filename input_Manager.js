...
class InputManager {
    constructor(scene) {
        ...
        this.pathfindingManager = new PathfindingManager(scene);
        ...
        scene.input.on('pointerdown', (pointer) => {
            ...
            this.pathfindingManager.setTarget(target);
        ...
        scene.input.on('pointermove', (pointer) => {
            ...
            if (this.isDragging) {
                const target = scene.cameras.main.getWorldPoint(pointer.x, pointer.y);
                this.pathfindingManager.setTarget(target); // Update target during drag
                if (scene.uiManager && typeof scene.uiManager.handlePointerMove === 'function') {
                    scene.uiManager.handlePointerMove(pointer.x, pointer.y, true);
                }
            }
    ...
    getDirection() {
        ...
        if (this.scene.isDialogOpen) {
            ...
            this.pathfindingManager.clearTarget();
            return { x: 0, y: 0 };
        }
        ...
        // Tap-to-move target
        if (dir.x === 0 && dir.y === 0) {
            dir = this.pathfindingManager.getDirection(this.scene.player);
        }
